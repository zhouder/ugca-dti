...
class DTIDataset(Dataset):
    """
    只从 cache 中读取：
    - molclr/{dataset}/{drug_id}.npz  → [Ld, 300]
    - esm2/{dataset}/{prot_id}.npz   → [Lp, 1280]
    """
    ...
    def load_feature_from_npz(self, subdir, file_id, expected_dim=None):
        """
        通用 npz loader，兼容不同 key: features/arr_0/data/...
        对于 MolCLR/ESM2 返回 2D: [L, D]；缺失时返回 [10, D] 的全零占位。
        """
        ...
        if not os.path.exists(path):
            ...
            if os.path.exists(path_alt):
                ...
                if expected_dim is not None:
                    # 对于序列特征，返回 [10, D] 占位（避免 pad_sequence 报错）
                    return torch.zeros(10, expected_dim)
    ...
    def __getitem__(self, idx):
        ...
        # 只读 MolCLR / ESM2
        molclr = self.load_feature_from_npz('molclr', did, 300)   # [Ld, 300]
        esm2 = self.load_feature_from_npz('esm2', pid, 1280)      # [Lp, 1280]
...
def collate_fn(batch):
    """
    把变长 MolCLR / ESM2 pad 成 batch：
      molclr: [B, Ld_max, 300]
      esm2:   [B, Lp_max, 1280]
      mask_d / mask_p: [B, Ld_max] / [B, Lp_max]
    """
    molclr_list = [x['molclr'] for x in batch]
    esm2_list = [x['esm2'] for x in batch]
    labels = torch.stack([x['label'] for x in batch]).unsqueeze(-1)  # [B, 1]

    molclr_pad = torch.nn.utils.rnn.pad_sequence(
        molclr_list, batch_first=True, padding_value=0.0
    )  # [B, Ld_max, 300]
    esm2_pad = torch.nn.utils.rnn.pad_sequence(
        esm2_list, batch_first=True, padding_value=0.0
    )   # [B, Lp_max, 1280]

    B = len(batch)
    mask_d = torch.zeros(B, molclr_pad.shape[1], device=molclr_pad.device)
    for i, seq in enumerate(molclr_list):
        mask_d[i, :seq.shape[0]] = 1.0

    mask_p = torch.zeros(B, esm2_pad.shape[1], device=esm2_pad.device)
    for i, seq in enumerate(esm2_list):
        mask_p[i, :seq.shape[0]] = 1.0
...
class DTIDataModule:
    ...
    def get_kfold_indices(self, df):
        ...
        if self.split_mode in ['warm', 'random']:
            ...
            return [
                (
                    df[df['smiles'].isin(unique[t])].index.to_numpy(),
            ...
            return [
                (
                    df[df['protein'].isin(unique[t])].index.to_numpy(),
